...
tf.app.flags.DEFINE_string('train_dir', '/cstor/xsede/users/xs-jdakka/tensorflow/3D_fMRI_CNN/tf_pipeline/results_sirp/experiment_4',
...
tf.app.flags.DEFINE_string('conv_layers', '2,1',
...
tf.app.flags.DEFINE_string('fold_to_run', '3,4',
                            """fold numbers to run (default=-1 for all folds).""")

tf.app.flags.DEFINE_integer('num_time_steps', 64,
...
class Trainer(object):
  def __init__(self, model, dataset):
    ...
    self.original_subject_IDs = None
    self.test_ind = None
  ...
  def load_data(self, random=False):
    ...
    if random:
      ...
      self.runs = np.random.permutation(range(1, 4)*95)
    ...
    # Load features
      ...
      self.original_subject_IDs = dictionary_IDs
  ...
  def load_data_test(self):
    ...
    g=h5py.File('shuffled_output_labels.hdf5','r')
    h=h5py.File('shuffled_output_subjects.hdf5','r')
    i=h5py.File('shuffled_output_features.hdf5','r')
   
    subjects_test, labels_test, features_test= [], [], []
   
    subjects_test=h['subjects'][()]
    labels_test=g['labels'][()]
    features_test=i['features'][()]

    # Load features
    features_test = np.expand_dims(np.array(features).transpose([4, 0, 3, 1, 2]),axis=2)  # Add another filler dimension for the samples
    
    #collect sites 

    # change labels from -1/1 to 0/1
    labels_test = (np.array(labels_test, dtype=int) == 1).astype(int)
    #labels[:10] = 1
    labels_test = [int(i) for i in labels_test]
    labels_test =np.asarray(labels_test)
    
    # map the self.subject_test values back to the self.original_subject_IDs to find the original subject number (before 1-95)
    # for every subject in self.subjects_test, find the val given the key in self.subjects_test and append to new_values
    # for every entry in new_values find the index in the subjects list from dataset #2

    old_subject_values = []
    for i in self.subjects_test:
      old_subject_values.append([key for key, val in self.original_subject_IDs.items() if val == i ])
  
    for i in old_subject_values:
      indices = [i for i,x in enumerate(subjects_test) if x == i]

    #self.subjects_test = np.squeeze(subjects_test[index]).astype(np.int32)
    self.x_test        = features_test[indices]
    self.y_test        = np.squeeze(labels_test[indices]).astype(np.int32)
    ...
    return [(self.x_test, self.y_test,self.subjects_test)]
  ...
  def preprocess_data(self):
    ...
    # Get rid of self.features in order to make room for loading test data
  ...
  def iterate_minibatches_test(self, subset):
    """

    :param subset:
    :return:
    """
    selector_dict = {'test': (self.x_test, self.y_test, self.subject_test)}
    inputs, targets, subjects = selector_dict[subset]
    input_len = inputs.shape[0]
    X = []
    L = []
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
      ...
      for epoch in xrange(FLAGS.num_epochs):
        ...
        if best_validation_acc < epoch_val_acc:
          ...
          epoch_acc, epoch_loss = 0, 0
          for batch_num, batch in enumerate(self.iterate_minibatches_test(subset='test')):
            inputs, targets = batch
            [l, acc] = sess.run([loss, batch_accuracy], feed_dict={inputs_ph: inputs, labels_ph: targets})
            epoch_acc += acc
            epoch_loss += l
...
def main(_):
  ...
  for fold_num, fold in enumerate([fold_pairs[i] for i in fold_to_run]):
    log_info_string('Beginning fold {0} out of {1}'.format(fold_to_run[fold_num] + 1, len(fold_pairs)))
    FLAGS.train_dir = os.path.join(train_dir, str(fold_to_run[fold_num]))
    
    print('Splitting the data...')
    tr.split_data(fold) #returns train, validation, test indices
